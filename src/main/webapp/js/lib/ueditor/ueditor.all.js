 ...
 * UEditor
 * version: ueditor
 * build: Wed Aug 17 2016 12:30:57 GMT+0800 (中国标准时间)
 */
...
//                        container.style.width = options.initialFrameWidth + 'px';
        ...
        _initEvents: function () {
            ...
            //by sweetyx 修复输入!@#$%等特殊字符时不触发contentChange的bug
            domUtils.on(doc, ['keyup'], function (evt) {
                if (evt.type == 'keyup' && (evt.ctrlKey || evt.metaKey || evt.shiftKey || evt.altKey)) {
                    me.fireEvent('contentchange');
                }
            });
        },
...
//                    (!cmd.ignoreContentChange && !me._ignoreContentChange) && me.fireEvent('contentchange');
        ...
        getActionUrl: function(action){
            return this.getOpt(action) || this.getOpt('fileUploadUrl') || '';
...
//                img.style.cssText += ';margin:' + (first.margin||0) +"px;" + 'border:' + (first.border||0) +"px solid #000";
            ...
            ci = opt[0];
            if (opt.length == 1) {
                str = '<img fullwidth="'+ (ci.notNeedFullScreen===true?'auto':'screen') +'" src="' + ci.src + '" ' + (ci._src ? ' _src="' + ci._src + '" ' : '') +
                ...
                for (var i = 0; ci = opt[i++];) {
                    str = '<p ' + (ci['floatStyle'] == 'center' ? 'style="text-align: center" ' : '') + '><img fullwidth="'+ (ci.notNeedFullScreen===true?'auto':'screen') +'" src="' + ci.src + '" ' +
...
UE.plugins['font'] = function () {
    ...
    me.setOpt({
        ...
        'fontsize': [12,13,14,15,16,17,18,20,22,24,30,36]
    });
                ...
                execCommand: function (cmdName, value) {
                    ...
                    if (value == 'default') {
                        ...
                        if (!range.collapsed) {
                            ...
                            range = me.selection.getRange();
                            //by sweetyx 背景色加padding
                            if(cmdName === 'backcolor'){
                                range.applyInlineStyle('span', {'style': style + ':' + value + ';padding:5px 2px;'});
                            }
                            else{
    ...
    me.commands['blockquote'] = {
        execCommand : function( cmdName, attrs ) {
            ...
            if ( obj ) {
                ...
                node.setAttribute('style','margin: 0.8em 0px 0.8em 2em;padding: 0px 0px 0px 0.7em;border-left-width: 2px;border-left-style: solid;border-left-color: #ff0000;');
                ...
                var childsP = domUtils.getElementsByTagName(node,'p');
                for(var i=0,ci;ci=childsP[i++];){
                    if(ci.parentNode){
                        domUtils.remove(ci,true);
                    }
                }
                var childsDiv = domUtils.getElementsByTagName(node,'div');
                for(var i=0,ci;ci=childsDiv[i++];){
                    if(ci.parentNode){
                        domUtils.remove(ci,true);
                    }
                }
...
UE.plugins['lineheight'] = function(){
    var me = this;
    me.setOpt({'lineheight':['1','1.25','1.5','1.75','2', '3', '4', '5']});
                   ...
                   if (img && img.getAttribute('anchorname')) {
                       ...
                       if (name) {
                           ...
                           domUtils.setAttributes(anchor,{
                               'anchorname':name,
                               'style':'background: url(http://yanxuan.nosdn.127.net/10911b3274fc3dca0e561e9c6d68e1b0.gif) no-repeat scroll left center transparent;cursor: auto;display: inline-block;height: 16px;width: 15px;'
                           });
...
(function (){
    ...
    UE.plugins['source'] = function (){
        ...
        me.commands['source'] = {
            execCommand: function (){
                ...
                if (sourceMode) {
                    ...
                    //做xss校验，by sweetyx
                    // if(typeof window.xssfilter.filterXSS === 'function'){
                    //     cont=window.xssfilter.filterXSS(cont);
                    // }
        ...
        //             src : "/scripts/third-party/codemirror/codemirror.js",
        //             tag : "script",
        //             type : "text/javascript",
        //             defer : "defer"
        //         },function(){
        ...
        //             rel : "stylesheet",
        //             type : "text/css",
        //             href : "/scripts/third-party/codemirror/codemirror.css"
        //         });
...
UE.plugins['video'] = function (){
    ...
    function creatInsertStr(url,width,height,id,align,classname,type,poster){
        ...
        switch (type){
            case 'image':
                str = '<img fullwidth="screen" ' + (id ? 'id="' + id+'"' : '') + ' poster="'+ poster +'" width="'+ width +'" height="' + height + '" _url="'+url+'" class="' + classname.replace(/\bvideo-js\b/, '') + '"'  +
                    ' src="http://yanxuan.nosdn.127.net/54c59ff5bc3d3689842dad279447fda7.gif" style="background:url(http://yanxuan.nosdn.127.net/93a8171ecc7b37fa52ea74f11b7a3298.gif) no-repeat center center; border:1px solid gray;'+(align ? 'float:' + align + ';': '')+'" />'
                break;
            case 'video':
                var ext = url.substr(url.lastIndexOf('.') + 1);
                if(ext == 'ogv') ext = 'ogg';
                str = '<video fullwidth="screen"' + (id ? ' id="' + id + '"' : '') + ' class="' + classname + ' video-js" ' + (align ? ' style="float:' + align + '"': '') +
    ...
    me.commands["insertvideo"] = {
        ...
        queryCommandState : function(){
            var img = me.selection.getRange().getClosedNode(),
                flag = img && img.className.indexOf("edui-upload-video")!=-1;
            return flag ? 1 : 0;
        }
    };
};
...
UE.plugin.register('autoupload', function (){
    ...
    function sendAndInsertFile(file, editor) {
        ...
        returnFileUrlName = me.getOpt('returnFileUrlName');
        ...
        if (filetype == 'image') {
            successHandler = function(data) {
                ...
                var imgHtml = '<img src="' + link + '" title="' + title + '" >';
                me.execCommand('inserthtml', imgHtml);
            };
        } else {
            successHandler = function(data) {
                var link = urlPrefix + data.url,
        ...
        xhr.addEventListener('load', function (e) {
            try{
                ...
                if (json.code === 200) {
                    //by sweetyx 返回的数据格式修改为编辑器所需的格式
                    var requiredData = {
                        name: json.data.file_name,
                        original: json.data.file_name,
                        size: file.size,
                        state: "SUCCESS",
                        type: file.type,
                        url: returnFileUrlName?json.data[returnFileUrlName]:json.data
                    };
                    successHandler(requiredData);
                } else {
                    errorHandler(json.error_msg);
                }
            }catch(er){
 ...
 * @author hzmailin
 *
 * @module  tool/xssfilter
 */
window.xssfilter = (function(){
    // 默认白名单
    var whiteList = {
        h1: [],
        h2: [],
    ...
    function StripTagBody(tags, next) {
        ...
        var isRemoveAllTag = false;//!$.isArray(tags);
    ...
    var DEFAULT = {};
    DEFAULT.whiteList = whiteList;
    DEFAULT.onTag = onTag;
    DEFAULT.onIgnoreTag = onIgnoreTag;
    DEFAULT.onTagAttr = onTagAttr;
    DEFAULT.onIgnoreTagAttr = onIgnoreTagAttr;
    DEFAULT.safeAttrValue = safeAttrValue;
    DEFAULT.escapeHtml = escapeHtml;
    DEFAULT.escapeQuote = escapeQuote;
    DEFAULT.unescapeQuote = unescapeQuote;
    DEFAULT.escapeHtmlEntities = escapeHtmlEntities;
    DEFAULT.escapeDangerHtml5Entities = escapeDangerHtml5Entities;
    DEFAULT.clearNonPrintableCharacter = clearNonPrintableCharacter;
    DEFAULT.friendlyAttrValue = friendlyAttrValue;
    DEFAULT.escapeAttrValue = escapeAttrValue;
    DEFAULT.onIgnoreTagStripAll = onIgnoreTagStripAll;
    DEFAULT.StripTagBody = StripTagBody;
        ...
        for (currentPos = 0; currentPos < len; currentPos++) {
            ...
            if (tagStart === false) {
                if (c === '<') {
                    ...
                    tagStart = currentPos;
                    if(currentPos < len - 3 && html.charAt(currentPos + 1) === '!' && html.charAt(currentPos + 2) === '-' && html.charAt(currentPos + 3) === '-'){
                        // 是注释的话跳过
                        var temp = html.slice(lastPos, len);
                        var tempIndex = temp.indexOf('-->');
                        if(tempIndex > 0){
                            rethtml += html.slice(lastPos, (lastPos + tempIndex + 3));
                            currentPos = (lastPos + tempIndex + 2);
        ...
        var retHtml = parseTag(html, function(sourcePosition, position, tag, html, isClosing) {
            ...
            if (info.isWhite) {
                ...
                var attrsHtml = parseAttr(attrs.html, function(name, value) {
                    ...
                    // 调用onTagAttr处理
                    var isWhiteAttr = ($.inArray(name, whiteAttrList || []) !== -1);
...
(function () {
    ...
    var iframeUrlMap = {
        'insertimage': UEDITOR_CONFIG.insertimageFrameUrl || '/ueditor/dialogs/image/image.html',
        'emotion': UEDITOR_CONFIG.emotionFrameUrl || '/ueditor/dialogs/emotion/emotion.html',
        'link': UEDITOR_CONFIG.linkFrameUrl || '/ueditor/dialogs/link/link.html',
        'anchor': UEDITOR_CONFIG.anchorFrameUrl || '/ueditor/dialogs/anchor/anchor.html',
        'insertvideo': UEDITOR_CONFIG.videoFrameUrl || '/ueditor/dialogs/video/video.html',
                                ...
                                if (dialog) {
                                    switch (cmd) {
                                        ...
                                        default:
                                            ...
                                            dialog.open();
                                            // sweetyx
                                            // if(dialog.iframeUrl.indexOf('video')>-1){
                                            //     $(dialog.getDom()).children().children().width(400).height(200);
                                            // }
                                    }
                                }
                            },
...
(function () {
    ...
    EditorUI.prototype = {
        ...
        initEditorUI:function () {
            ...
            editor.addListener('ready',function(){
                //by sweetyx
                //修复源码编辑状态下不触发编辑事件的bug
                var iframe = $(editor.iframe);
                iframe.parent().on('blur', 'textarea', function(evt) {
